...
class ObdFaultListAdapter(data :MutableList<ObdData>):BaseQuickAdapter<ObdData, BaseViewHolder>(R.layout.item_data_info_2,data) {

    var index = 999

    override fun convert(holder: BaseViewHolder, item: ObdData) {

//        if (item.key == "Unsettled" || item.key.contains("UnsetDTC")
//            || item.key.contains("UnsetDetail")
//            || item.key == "Mileage"
//            || item.key == "MILTime"){
